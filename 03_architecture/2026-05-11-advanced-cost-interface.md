...
## 1. 개요 (Overview)
정유 공정의 물류 흐름(`자재이동문서`)에 따라 변동되는 재고 자산의 수량과 가치를 재무/원가 장부와 실시간으로 일치시키기 위한 전사 데이터 연동 아키텍처입니다. 

기존 [system-flow-v4.md](system-flow-v4.md) 에서 정의된 전체 마스터 플로우를 기반으로 하되, 본 문서에서는 물류 트랜잭션 발생 시 자재 마스터(ZTB1MM0001 테이블)의 **고정 생산원가(Cost)**와 저장위치별 재고(ZTB1MM0003 테이블)의 **총 재고량 및 총 평가금액** 필드가 상호 작용하는 세부 인터페이스 로직을 명시합니다.

---

## 2. 중앙 통제형 데이터 흐름도 (Data Architecture)

ZTB1MM0001 및 ZTB1MM0003 테이블을 직접 수정할 수 없으며, Member 1(PM)이 개발한 공통 함수 모듈을 호출하여 데이터 정합성을 방어합니다.
...
본 인터페이스 아키텍처를 통해 영업/판매(SD) 모듈이 글로벌 시황(MOPS)에 따라 판매 가격(Price)을 다이내믹하게 변동시켜 매출을 일으키더라도, 시스템 내부의 출고 원가는 ZTB1MM0001 테이블에 고정된 **'생산원가(Cost)'**를 기준으로 안정적으로 차감됩니다. 

생산단 역시 CO-PP가 도출한 1차 계산 공정비용을 자재이동 시 발생하는 가치 금액으로 다이렉트 매핑하여, 특정 이동유형을 통해 공정비용 G/L 계정과 연동되도록 완벽히 설계하였습니다.
결과적으로 복잡한 전표 전사 리스크 없이 공통 함수의 [ZTB1MM0003 가용재고 수량 차감 × ZTB1MM0001 고정 생산원가] 공식에 의해 전사 재고 정합성과 물류 거점별 마진 분석 체계를 심플하고 명확하게 완성하였습니다.
