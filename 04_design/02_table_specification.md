...
## 2026-04-23 PP 테이블 스펙 변경 내역

### 1. 헤더 테이블 (`ZTB1PP0013`) 및 아이템 테이블 (`ZTB1PP0014`) 구조 개편
* **PK 변경**: 계획과 오더를 명확히 분리하기 위해 `PLNTY`(계획/오더 구분자) 필드를 두 테이블의 Primary Key로 추가 지정.
* **필드 정비 및 명명 규칙 통일**:
    * 아이템 테이블(`ZTB1PP0014`)의 `REQTY`(요구수량) 필드를 `POQTY`(오더수량)로 변경하여 계획/실제 생산 수량 연결 기준 정립.
    * 불필요 필드인 `ACQTY`(실제수량), `DATUB`(종료일자) 삭제.
    * 헤더 테이블(`ZTB1PP0013`)에서 중복 확인 가능한 BOM 관련 필드(`STLNR`, `STLKN`)를 아이템 테이블에서 삭제하여 데이터 정규화 수행.

### 2. Data Element (DDIC) 변경 사항
* **삭제**: `ZEB1_PP_REQTY`, `ZEB1_PP_ACQTY`
* **신규 생성**: `ZEB1_PP_POQTY` (오더 수량 전용 엘리먼트)
